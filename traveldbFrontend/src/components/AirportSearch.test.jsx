...
describe("AirportSearch", () => {
  ...
  it("loads the next page after the current result count", async () => {
    ...
    expect(searchAirports).toHaveBeenLastCalledWith("pagination-route-test", expect.objectContaining({
      limit: 50,
      offset: 1,
    }));
    expect(screen.getByRole("option", { name: /London Heathrow Airport/ }))
      .toBeInTheDocument();
  });
  ...
  it("ignores a stale page rejection after the query changes", async () => {
    ...
    expect(screen.queryByText("More airports could not be loaded. Try again."))
    ...
    expect(screen.getByRole("option", { name: /Charles de Gaulle Airport/ }))
      .toBeInTheDocument();
  });
});
