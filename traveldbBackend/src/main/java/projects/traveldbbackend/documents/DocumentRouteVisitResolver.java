...
final class DocumentRouteVisitResolver {
        ...
        while (runStart < route.size()) {
            ...
            if (runStart == 0 && "SCHENGEN".equals(jurisdictionCode)) {
                ...
                for (int index = 1; index <= runEnd; index++) {
                    ...
                    if (!originCountry.equals(countryCode(airport))) {
                        visits.add(new CountryVisit(countryCode(airport), airport.getIataCode(), Scope.ENTRY));
                        break;
                    }
                }
            }
            ...
            if (runStart > 0) {
                ...
                boolean crossesBorder = runEnd == route.size() - 1
                        || runEnd > runStart
                        || forcedEntryAirports.contains(firstArrival.getIataCode());
                Scope scope = crossesBorder ? Scope.ENTRY : Scope.TRANSIT;
                visits.add(new CountryVisit(countryCode(firstArrival), firstArrival.getIataCode(), scope));
            }
    ...
    private static String countryCode(Airport airport) {
        if (airport == null || airport.getCountryCode() == null) return "";
        return airport.getCountryCode().trim().toUpperCase(Locale.ROOT);
    }

    private static String jurisdictionCode(Airport airport) {
        return airport != null && airport.isSchengen() ? "SCHENGEN" : countryCode(airport);
    }
