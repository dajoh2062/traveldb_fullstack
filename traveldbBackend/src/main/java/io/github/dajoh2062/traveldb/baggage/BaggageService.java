...
public class BaggageService {
    ...
    public static final String GUIDANCE_REVIEWED_DATE = "2026-07-31";
    ...
    public BaggageCheckResult check(BaggageCheckRequest request) {
        BaggageCheckResult result = new BaggageCheckResult();
        ...
        for (int i = 1; i < request.route().size() - 1; i++) {
            Airport previous = request.route().get(i - 1);
            Airport current = request.route().get(i);
            Airport next = request.route().get(i + 1);
            result.addBaggageAdvice(adviceFor(request, previous, current, next));
        }
        return result;
    }

    private BaggageAdvice adviceFor(BaggageCheckRequest request, Airport previous, Airport current, Airport next) {
        boolean enteringCountry = !sameCountry(previous, current);
        boolean onwardDomestic = sameCountry(current, next);

        if (enteringCountry && "US".equals(current.countryCode())) {
            return usEntryAdvice(request, previous, current);
        }
    ...
    private BaggageAdvice usEntryAdvice(BaggageCheckRequest request, Airport previous, Airport current) {
        if (US_PRECLEARANCE_AIRPORTS.contains(previous.iataCode())) {
            if (request.throughCheckStatus() == BaggageCheckRequest.ThroughCheckStatus.NO) {
                return required(
                        current,
